trigger PNRcTrigger on PNR__c (before update, after update) {

    List<Segment__c> segs = new List<Segment__c>();
    List<PNR_Accounting_Line__c> pals = new List<PNR_Accounting_Line__c>();
    ...
    if (Trigger.isAfter && Sabre_Constants.runOnce) {
		List<PNR__c> pnrListToUpdate = new List<PNR__c>();
        for (PNR__c pnr : [Select Id, XML_Response__c,Callout_Status_Code__c,Is_Response_Parsed__c, PNR_Sequence__c,Creation_Agent_Id__c,Base_Price__c, Base_Price_Currency__c, Equiv_Price__c, Equiv_Price_Currency__c,Total_Tax__c, Total_Tax_Currency__c, Total_Price__c, Total_Price_Currency__c from PNR__c  where Id IN : Trigger.new ]) {
            if (pnr.XML_Response__c != null && pnr.Callout_Status_Code__c == '200'  && pnr.Is_Response_Parsed__c == false){
                    ...
                    if (als.accountingLines != null) {
                        if (als.accountingLines.size() > 0) {
                            for (SabreReservationRes.AccountingLine al : als.accountingLines) {
                                PNR_Accounting_Line__c pal = new PNR_Accounting_Line__c();
                                pal.pnr__c = pnr.Id;
